...
static const int LAST_POW_BLOCK = 10;
...
static const int64_t MAX_MINT_PROOF_OF_STAKE = 0.01 * COIN;	// low interest of 1%
...
public:
    ...
    {
        // tenbillion: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // tenbillion: get transaction coin age
...
public:
    ...
    // tenbillion: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // tenbillion: two types of block: proof-of-work or proof-of-stake
    ...
    // tenbillion: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // tenbillion: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // tenbillion: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // tenbillion: block index flags
    enum  
    {
